...
void LinkedList<T>::insert(int index, T entry) {
	...
	if (index == 0) {
		...
		for (int i = 0; i < m_length - 1; i++) temp2 = temp2->getNext();
		temp2->setNext(temp);
	} else {
		Node<T>* temp2 = m_front;
		for (int i = 0; i < index - 1; i++) temp2 = temp2->getNext();
...
template <typename T>
bool LinkedList<T>::contains(T entry) const {
	Node<T>* temp = m_front;
	while (temp != nullptr) {
		if (temp->getEntry() == entry) return true;
		temp = temp->getNext();
	}
	return false;
}